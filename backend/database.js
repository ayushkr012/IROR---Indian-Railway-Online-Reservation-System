...
const routeInfo = async (id) => {
  try {
    const data = pool.query(
      `SELECT CURRENTSTATION, TIMEFROMSTART FROM ROUTES WHERE TRAINID=? AND ROUTEID IN (SELECT MIN(ROUTEID) FROM ROUTES) ORDER BY TIMEFROMSTART`,
      [id]
    );
    return data;
  } catch (error) {
...
const getStStation = async (id) => {
  try {
    const data = await pool.query(
      `SELECT CURRENTSTATION FROM ROUTES WHERE TRAINID=? AND TIMEFROMSTART IN (SELECT MIN(TIMEFROMSTART) FROM ROUTES);`,
      [id]
    );
...
const getEndStation = async (id) => {
  try {
    const data = await pool.query(
      `SELECT CURRENTSTATION FROM ROUTES WHERE TRAINID=? AND TIMEFROMSTART IN (SELECT MAX(TIMEFROMSTART) FROM ROUTES);`,
      [id]
    );
